...
/* 2
Note the INNER JOIN misses the teachers 
with no department and the departments with no teacher.
*/
...
/* 7
Use COUNT to show the number of teachers 
...
and the number of staff. Use a RIGHT JOIN to ensure that 
the Engineering department is listed.
*/

SELECT COALESCE(dept.name,'Engineering'), COUNT(teacher.id) FROM teacher
